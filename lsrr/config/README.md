...
## 레거시 참조
`Legacy_LSRR/lsrr/config.py` — **개작**. 계층 해석·dotlist·스윕 전개 로직은 건전하므로 파일 분할해 이식. 단 문자열 섹션을 자동으로 파일로 해석하던 암묵 규칙(`config.py:105-116`)은 `schema.py`의 명시적 선언으로 대체한다 — 암묵 규칙은 설정 오타를 조용히 삼킨다.
