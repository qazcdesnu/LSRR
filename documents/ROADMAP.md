...
> 이 문서는 **무엇을 어떤 순서로 만들 것인가**를 다룬다. 무엇을 주장하는가는 `Research_Proposal_v2.1.md`, 어떻게 조립되는가는 `ARCHITECTURE.md`.
> 원칙: **게이트 이전에는 최소한만 만든다.** Phase 0 게이트 ③이 킬 스위치이므로, 그 판정에 필요 없는 것은 만들지 않는다.

**최종 갱신:** 2026-09-12 · 현재 위치: **2단 ③ FAIL — 킬 스위치 발동 (F-034). 재검토 중: 감쇠 갱신 복원 완료 (F-035·F-036, ADR-017 개정), 다음은 F-030(정제의 샘플 의존성)**
...
**③ 은 2단 구조다 — 1단은 기준선, 2단이 킬 스위치 (ADR-016 재개정).**

| 단계 | 구성 | 역할 | 결과 |
|---|---|---|---|
| 1단 | Phase A 동결, 콜드 스타트 | **기준선** — ①·④ 판정, ②·③ 측정만 | `check_gates.py --stage baseline` |
| 2단 | 1단 체크포인트 + LoRA 디코딩 정렬 (임계값 동일) | **킬 스위치** | FAIL → **최종 킬** — 아키텍처 재검토 |

근거: 학습되지 않은 것은 벡터가 아니라 **수신기**다. 동결 백본은 연속 벡터를 몇 개 받든 분포 밖 입력을 최대한 활용하는 것 이상은 못 하므로, 앵커 없는 토큰이 늘수록 교란만 는다 — 1단에서 궤적 방출이 이길 이유가 애초에 없다. 퍼즈 토큰(학습된 임베딩 + 사전학습)과 Coconut(백본 미세조정)이 같은 사실 위에 서 있다.

①·④는 백본에 닿기 전의 엔진 상태만 보므로 1단에서 판정한다. **②·③은 2단에서 판정한다** — anytime 도 백본을 통과해 정확도로 나오므로 수신 병목을 피할 수 없다 (F-033).
...
*레거시 조치:* 완료 — 레거시가 참조 가치를 갖는 모든 영역(엔진 수치, 평가 프로토콜, 데이터 스키마, 개입 훅)의 신규 구현이 검증을 마쳤고, 작업 트리의 사본은 2026-09-11 에 들어냈다. 참조가 필요하면 `v1.0` 브랜치를 본다.
...
```
M0 ─▶ M1 ─▶ M2 ─▶ M3 ─▶ M4 ─▶ M5 ─▶ [1단: Phase A 기준선] ─▶ [2단: + Phase B LoRA]
                             │            ②·④ 판정                    ③ 킬 스위치
                    레거시 runs/caches 삭제                                 │
                                                                 ③ FAIL ─▶ 아키텍처 재검토 (최종 킬)
                                                                    │
...
> **레거시는 `v1.0` 브랜치에만 있다** (커밋 `8a3fe34`, 그 브랜치의 저장소 루트가 곧 레거시 구현). 작업 트리 사본과 산출물은 모두 들어냈다. 꺼내 볼 때는 `git show v1.0:<경로>`.
>
> **`main` 은 더 이상 레거시가 아니다** — 2026-09-11 에 `main` 을 v1.1 로 fast-forward 했다. 브랜치 운용은 CONVENTIONS §8.

| 조치 | 회수 | 상태 | 실행일 |
|---|---:|---|---|
| 레거시 `runs/`·`caches/` 삭제 | 29 GB | **완료** | 2026-09-11 |
| 작업 트리 `Legacy_LSRR/` 삭제 | 1.6 MB | **완료** | 2026-09-11 |
| `v1.0` 브랜치로 보존 | — | **완료** | 2026-09-11 |

- 삭제 근거: 엔진 이식 충실도 테스트(`tests/unit/test_hydra_port_fidelity.py`)가 통과해 레거시 체크포인트의 대조 가치가 소멸했고, `LEGACY_MAP.md` §1·§2 의 이식/개작 항목이 전부 `검증` 상태가 됐다.
- **게이트 ③ 재검토 중에도 문제없다.** 레거시를 독립 대조 구현으로 다시 봐야 하면 `git show v1.0:<경로>` 로 즉시 꺼낼 수 있다 — 브랜치를 지우지 않는 이유다.
- 아카이브는 만들지 않는다. 브랜치가 그 역할을 한다.
