...
**최종 갱신:** 2026-09-12 · 현재 위치: **게이트 1단(기준선) 스윕 진행 중 → 2단(킬 스위치) 대기**
...
| 단계 | 구성 | 역할 | 결과 |
|---|---|---|---|
| 1단 | Phase A 동결, 콜드 스타트 | **기준선** — 수신 병목 비용 측정 | 판정하지 않는다 (`check_gates.py --stage baseline`) |
...
게이트 ②·④는 수신기와 무관한 엔진 측 성질이므로 1단 데이터로 판정한다.
