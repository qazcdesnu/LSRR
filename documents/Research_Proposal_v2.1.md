...
$$h^{(m)} = s \cdot \mathrm{RMSNorm}\!\left( W_{\text{proj}} \cdot h_{\text{ssm}}^{(m)} \right) \in \mathbb{R}^d$$

여기서 $s$는 백본 입력 임베딩의 성분 RMS에서 출발하는 학습 스칼라다. 최종 방출되는 잠재 사고 토큰 시퀀스는 $M$개의 연속 벡터로 구성된다:
...
#### 잔차 앵커 $h_T^{(L)}$를 더하지 않는 이유 (v2.1 개정)
...
**더 중요하게, 앵커는 모든 사이클에 같은 벡터를 더하는 공통 스탬프이므로 궤적의 분화를 원천에서 막는다.** 실측(M=5, GPT-2, 1자리 곱셈):

| 방출식 | 학습 후 NLL | 토큰간 평균 코사인 | 셔플 개입 $\Delta$NLL |
|---|---:|---:|---:|
| 앵커 있음 (v1) | 0.2956 | 0.9680 | 2.71 |
| **앵커 없음 (v2)** | **0.0015** | 0.8835 | **7.12** |

앵커가 있으면 학습 **전** 토큰간 코사인이 $1.0000$(유효 랭크 1.00)으로 M개 토큰이 사실상 하나다. 앵커를 빼면 NLL이 197배 낮아지고, 셔플 개입 $\Delta$가 7.12로 궤적이 질문별 정보를 실제로 나른다. 접두 길이별 NLL도 앵커 없음이 $k{=}2$에서 이미 0.30에 도달하는 반면, 앵커 있음은 $k{=}5$를 다 써야 같은 값에 이른다.

이로써 잠재 토큰의 존재 이유가 **"질문에 없는 것, 즉 SSM이 만든 정제 내용의 운반"**으로 순수해진다. Coconut의 연속 사고 역시 앵커 없이 은닉 상태를 그대로 주입한다는 점에서 선례와도 정합한다.
...
**단, 예비 실측은 다른 양상을 보인다.** 앵커 없음 조건에서 토큰별 기여($h^{(m)}$만 영벡터로 치환했을 때의 $\Delta$NLL)는 다음과 같았다.

$$m_1{=}1.11,\quad m_2{=}0.03,\quad m_3{=}0.07,\quad m_4{=}0.28,\quad m_5{=}4.15$$

기여가 **양끝에 몰리고 중간 구간이 빈다.** 후반 유사가 아니라 중간 공동(空洞)이다. §5.2의 $L_{\text{VarReg}}^{\text{explore}}$가 초기 $\lceil M/4 \rceil$ 구간을 겨냥한 설계인 것과 어긋나는 방향이다. 다만 이 관측은 1자리 곱셈(64개 조합)에서 얻은 것으로, $M{=}5$를 요구하지 않는 과제라 궤적이 남아도는 상황일 수 있다. **다자리 곱셈 및 ProsQA에서 재확인이 필요하며**, 재확인 결과에 따라 $L_{\text{VarReg}}$의 적용 구간을 초기에서 중간으로 옮기거나 유효 다양성 기반 궤적 길이 결정을 검토한다. [Ablation-마킹: VarReg 적용 구간]
