...
- **문서 오류 수정은 예외로 즉시 병합한다.** 진전을 기다리지 않는다. 근거는 다른 변경과 성질이 다르기 때문이다 — 미완성 기능이 `main` 에 없는 것은 "아직 안 됐다" 지만, **틀린 문서가 `main` 에 있는 것은 읽는 사람을 잘못된 곳으로 보낸다.** 지연에 이득이 없고 손해만 있다.
  - 해당: 죽은 경로·깨진 링크, 뒤집힌 결정이 반영되지 않은 서술, 실제와 다른 상태 표기.
  - 비해당: 새 ADR·FINDINGS 항목처럼 **내용이 늘어나는** 변경. 이건 그 결정·측정이 속한 진전과 함께 올린다.
