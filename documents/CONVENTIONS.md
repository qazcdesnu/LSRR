...
**레거시는 `v1.0` 브랜치에만 있다.** 작업 트리의 `Legacy_LSRR/` 사본은 2026-09-11 에 삭제했다.

- 참조는 자유롭게 한다. 재현 검증(수식이 맞는지, 엣지 케이스가 무엇이었는지)에 특히 유용하다:

  ```bash
  git show v1.0:lsrr/engines/hydra_qs.py     # 파일 하나
  git diff v1.0 main -- lsrr/engine/         # 이식·개작의 실제 차이
  ```

- 코드를 가져올 때는 **이식(그대로) / 개작(수정) / 폐기(버림)** 중 하나로 판정하고 `documents/LEGACY_MAP.md`에 기록한다.
- 가져온 파일 상단에 출처를 남긴다: `# 이식: v1.0:lsrr/engines/hydra_qs.py (개작: ...)`. 이 표기는 그대로 `git show` 인자다.
- **v1.0 을 체크아웃해 수정하지 않는다.** 보존 브랜치이지 작업 브랜치가 아니다 (§8).
- 레거시의 `runs/`·`caches/` 산출물은 현재 코드베이스의 결과와 **섞어 보고하지 않는다**. 디코딩 경로가 다르므로(ADR-001) 비교 불가다. 해당 산출물은 이미 삭제했다 (29 GB 회수).
